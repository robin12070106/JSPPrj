...
public class NoticeDelController extends HttpServlet{
		...
		protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
			...
			if(session.getAttribute("id")==null)
				response.sendRedirect("../account/login?return-url=../customer/notice-detail?code="+code);
			
			else if(!session.getAttribute("id").equals(n.getWriter()))
				response.getWriter().println("<script>alert('권한이 없습니다');location.href='notice-detail?code="+code+"'</script>");
